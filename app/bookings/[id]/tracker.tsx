...
const STEPS = [
  ...
  { key: "PAID", label: "Payment credited" },
] as const;
...
export default function Tracker({ initial }: { initial: Data }) {
    ...
    <main className="mx-auto min-h-screen max-w-md p-5 pb-16">
      <header className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-400">
            {b.date} · {b.slot.windowStart}–{b.slot.windowEnd} · {b.crop}, {b.quantityQuintals} qtl
          </p>
        </div>
      ...
      {terminal ? (
        <div className={`rounded-2xl p-4 text-sm font-medium ${b.status === "NO_SHOW" ? "bg-red-50 text-red-700" : "bg-gray-100 text-gray-600"}`}>
          {b.status === "NO_SHOW"
            ? "You were marked absent at the centre. Please book a new slot."
            ...
            return (
              <li key={s.key} className="relative flex gap-3 pb-5 last:pb-0">
                ...
                <div className="min-w-0">
                  <p className={`text-sm font-medium ${done || current ? "text-gray-900" : "text-gray-400"}`}>{s.label}</p>
                  {s.key === "ARRIVED" && servedNow ? (
                    <p className="mt-0.5 inline-flex items-center gap-1 rounded bg-violet-50 px-1.5 py-0.5 text-xs font-medium text-violet-700">
                      <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-violet-500" /> Being served now
      ...
      <section className="mt-4 rounded-2xl bg-white p-4 text-sm ring-1 ring-gray-100">
        <div className="flex justify-between">
          <span className="text-gray-500">
            {b.crop} · {b.quantityQuintals} qtl × {inr(b.ratePerQuintal)}
          </span>
